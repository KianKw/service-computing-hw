...
var githubResthttp http.Handler
...
func loadResthttp(routes []route) http.Handler {
	h := httpRouterHandle
...
func BenchmarkGithubALLAPI(b *testing.B) {
	benchRoutes(b, githubResthttp, githubAPI)
}

func init() {
	println("#GithubAPI Routes:", len(githubAPI))

	calcMem("Resthttp", func() {
		githubResthttp = loadResthttp(githubAPI)
	})
	println()
}
